...
class DQNAgent:
    ...
    def __init__(self, state_size, action_size):
       ...
       # Modify here
        ...
        self.target_update_frequency = 5#1
...
###############################################################################
    ...
    def train_model(self):
       ...
       # Added by me 
        y_batch = []
        for i in range(self.batch_size): # Setting yj and gradient descent  line 11-12
...
if __name__ == "__main__":
    ...
    # Line 1 of Pseudo Code
    #Create agent, see the DQNAgent __init__ method for details
    agent = DQNAgent(state_size, action_size)

    #Collect test states for plotting Q values using uniform random policy
    test_states = np.zeros((agent.test_state_no, state_size))
    #Line 2 of Psuedo Code
    ...
    for e in range(EPISODES): # Line 3 of pseudo code
        done = False
        score = 0
        
        #Line 4 of Pseudo Code
        ...
        #line 5 of Pseudo Code
        while not done:
            if agent.render:
                env.render() #Show cartpole animation

            #Get action for the current state and go one step in environment
            action = agent.get_action(state) #Line 6 of Pseudo Code
            next_state, reward, done, info = env.step(action) # Line 7 of Pseudo Code
            next_state = np.reshape(next_state, [1, state_size]) #Line 8 of PseudoCode

            #Save sample <s, a, r, s'> to the replay memory
            agent.append_sample(state, action, reward, next_state, done)  # Line 9 of Pseudo Code
            #Training step
            agent.train_model() # Line 11 and 12 of Pseudo Code
